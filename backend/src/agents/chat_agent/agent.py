        ...
        for attempt in range(1, max_retries + 1):
            try:
                ...
                session = await self.session_service.get_session(
                    app_name=self.app_name, user_id=user_id, session_id=str(chapter_id)
                )
                ...
                error_msg = (
                    "Agent did not give a final response. Unknown error occurred."
                )
                if attempt >= max_retries:
                    raise Exception(error_msg)
                last_error = error_msg
                ...
                if debug:
                    print(
                        f"[RETRY] Attempt {attempt} failed, retrying in {retry_delay} seconds... Error: {last_error}"
                    )
