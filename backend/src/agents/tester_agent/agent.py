...
def get_full_instructions(
    ...
    files.extend(
        [
            f"explainer_agent/plugin_docs/{filename}"
            for filename in os.listdir(
                os.path.join(os.path.dirname(__file__), "plugin_docs")
            )
        ]
    )
...
class InitialTesterAgent(StructuredAgent):
    def __init__(self, app_name: str, session_service):
        ...
        tester_agent = LlmAgent(
            ...
            output_schema=Test,
            global_instruction=lambda _: load_instruction_from_file(
                "tester_agent/instructions.txt"
            )
            + "\n"
...
class TesterAgent(StandardAgent):
    ...
    def __init__(self, app_name: str, session_service, iterations: int = 2):
        self.inital_tester = InitialTesterAgent(
    ...
    async def _review_and_correct_question(
        self, question: Dict[str, Any], user_id: str, state: dict
        ...
        for i in range(self.iterations):
            ...
            response = await self.code_review.run(
                user_id=user_id, state=state, content=content
            )
        ...
        print(
            f"!!ERROR: Could not fix code for a question after {self.iterations} iterations. Discarding question."
        )
        return None

    async def run(
        self, user_id: str, state: dict, content: types.Content, debug: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        initial_response = await self.inital_tester.run(
            user_id=user_id, state=state, content=content
        )
