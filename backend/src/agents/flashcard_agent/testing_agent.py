...
class TestingFlashcardAgent(StandardAgent):
    ...
    async def generate_questions(
        ...
        try:
            ...
            response = await self.run(
                user_id="system", state={}, content=create_text_query(prompt)
            )
    ...
    async def _generate_questions_from_chunks(
        ...
        chunks = self._split_text_into_chunks(
            text_content, chunk_size=8000, overlap=500
        )
        ...
        for i, chunk in enumerate(chunks):
            ...
            if i == len(chunks) - 1:  # Last chunk gets remaining questions
                chunk_questions = num_questions - (
                    questions_per_chunk * (len(chunks) - 1)
                )
    ...
    def _split_text_into_chunks(
        ...
        while start < len(text):
            ...
            if end < len(text):
                ...
                if (
                    break_point > start + chunk_size // 2
                ):  # Only if break point is reasonable
