...
async def login_user(
    form_data: OAuth2PasswordRequestForm, db: Session, response: Response
) -> auth_schema.APIResponseStatus:
...
async def admin_login_as(
    current_user_id: str, user_id: str, db: Session, response: Response
) -> auth_schema.APIResponseStatus:
    """
    ...
    if user.is_admin:
        logger.warning(
            "Attempted to log in as admin user: %s (ID: %s)", user.username, user.id
        )
        raise HTTPException(
...
async def register_user(
    user_data: user_schema.UserCreate, db: Session, response: Response
) -> auth_schema.APIResponseStatus:
...
async def logout_user(
    user: user_schema.User, db: Session, response: Response
) -> auth_schema.APIResponseStatus:
...
async def refresh_token(
    token: Optional[str], db: Session, response: Response
) -> auth_schema.APIResponseStatus:
    """Registers a new user and returns the created user data."""
...
async def handle_oauth_callback(request: Request, db: Session, website: str = "google"):
    ...
    if picture_url:
        try:
            ...
            profile_image_base64_data = base64.b64encode(response.content).decode(
                "utf-8"
            )
    ...
    if not db_user:
        logger.info(
            f"Creating new user for {website} OAuth login: %s (%s)", email, name
        )
        # If the user does not exist, create a new user
        base_username = (
            name.lower().replace(" ", ".")[:40]
            if name
            else (email.split("@")[0][:40] if email else "user")
        )
        ...
        logger.info(
            f"Use existung user %s from database for {website} OAuth login.",
            db_user.username,
        )
        # If the user exists, update their details if necessary
        if (
            profile_image_base64_data
            and getattr(db_user, "profile_image_base64", None)
            != profile_image_base64_data
        ):
