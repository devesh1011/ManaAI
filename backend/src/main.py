...
output_dir = (
    Path("/tmp/anki_output") if os.path.exists("/tmp") else Path("./anki_output")
)
...
async def read_users_me(
    current_user: Optional[user_model.User] = Depends(auth.get_current_user_optional),
):
...
async def root():
    ...
    return {
        "message": "Welcome to the User Management API. API endpoints are under /api"
    }
