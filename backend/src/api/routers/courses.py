...
async def course_websocket(
    ...
    try:
        ...
        try:
            ...
            logger.error(
                f"Course access verification failed for user {user_id}, course {course_id}: {e}"
            )
        ...
        await ws_manager.connect(websocket, str(user_id), course_id)
        ...
        # If we haven't authenticated yet, just close
        pass
    except Exception as e:
        logger.error(f"WebSocket error for course {course_id}: {e}")
        try:
            ws_manager.disconnect(
                websocket,
                str(user_id) if "user_id" in locals() else "unknown",
                course_id,
            )
        except:
            pass
