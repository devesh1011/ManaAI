...
def get_flashcard_service() -> FlashcardService:
    ...
    if flashcard_service is None:
        ...
        flashcard_service = FlashcardService("mana ai", session_service)
...
async def analyze_pdf(
    ...
    try:
        ...
        if preview is None:
            raise HTTPException(
                status_code=404, detail="Document not found or analysis failed"
            )

        # Convert to response format
        response = AnalyzeResponse(
            estimated_cards=preview.estimated_cards, chapters=preview.chapters or []
        )
...
async def download_flashcards(
    ...
    if file_path is None or not os.path.exists(file_path):
        raise HTTPException(
            status_code=404, detail="File not found or task not completed"
        )
...
async def retry_task(
    ...
    return TaskActionResponse(
        task_id=task_id, status="pending", message="Task retry initiated"
    )
...
@router.get("/config")
async def get_upload_config(current_user: User = Depends(get_current_active_user)):
