...
export const chatService = {
  ...
  sendMessage: async (courseId, chapterId, message, onProgress) => {
    try {
      ...
      console.log(
        "Sending request to:",
        `${apiWithCookies.defaults.baseURL}${url}`
      );
      ...
      if (apiWithCookies.defaults.headers.common?.Authorization) {
        headers["Authorization"] =
          apiWithCookies.defaults.headers.common.Authorization;
      }
  ...
  getChatHistory: async (courseId, chapterId) => {
    try {
      const response = await apiWithCookies.get(
        `/chat/history/${courseId}/${chapterId}`
      );
