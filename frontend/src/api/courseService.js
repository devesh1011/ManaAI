...
export const courseService = {
  ...
  getCourseChapters: async (courseId) => {
    const chapters = (await apiWithCookies.get(`/courses/${courseId}/chapters`))
      .data;
  ...
  getChapter: async (courseId, chapterId) =>
    (await apiWithCookies.get(`/courses/${courseId}/chapters/${chapterId}`))
      .data,

  // Get questions for a specific chapter with full QuestionResponse data
  getChapterQuestions: async (courseId, chapterId) =>
    (await apiWithCookies.get(`/chapters/${courseId}/chapters/${chapterId}`))
      .data,
  ...
  getQuestionFeedback: async (courseId, chapterId, questionId, userAnswer) => {
    return (
      await apiWithCookies.get(
        `/chapters/${courseId}/chapters/${chapterId}/${questionId}/feedback?users_answer=${encodeURIComponent(
          userAnswer
        )}`
      )
    ).data;
  ...
  updateCourse: async (courseId, title, description) => {
    ...
    const response = await apiWithCookies.put(
  ...
  createCourse: async (data) => {
    // Removed onProgress, onError, onComplete
    ...
    console.log(
      "[POST] Course creation request successful, response:",
      response.data
    );
    return response.data;
  },
